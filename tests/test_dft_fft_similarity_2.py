...
def compare_dft_fft_results():
    ...
    diff_recursive = np.abs(dft_result - fft_recursive_result)
    diff_iterative = np.abs(dft_result - fft_iterative_result)
    diff_fft = np.abs(fft_recursive_result - fft_iterative_result)
    ...
    # Plot DFT results
    plt.subplot(3, 1, 1)
    plt.stem(freqs[:N // 2], np.abs(dft_result[:N // 2]), linefmt='b-', markerfmt='bo', basefmt='r-', label="DFT", use_line_collection=True)
    ...
    plt.subplot(3, 1, 2)
    plt.stem(freqs[:N // 2], np.abs(fft_recursive_result[:N // 2]), linefmt='g-', markerfmt='go', basefmt='r-', label="Recursive FFT", use_line_collection=True)
