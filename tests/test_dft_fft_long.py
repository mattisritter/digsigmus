...
def compare_dft_fft():
    ...
    # Measure DFT
    print("Running DFT...")
    dft_result, dft_time, dft_memory = measure_memory_and_time(dft_optimized, signal_dft)
    ...
    fft_iterative_result, fft_iterative_time, fft_iterative_memory = measure_memory_and_time(fft_iterative, signal_padded_iterative)

    # Print Results
    print(f"DFT (N={len(signal_dft)}): Zeit = {dft_time:.6f} s, Speicher = {dft_memory / 1024:.2f} KiB")
    print(f"Rekursive FFT (N={len(signal_padded_recursive)}): Zeit = {fft_recursive_time:.6f} s, Speicher = {fft_recursive_memory / 1024:.2f} KiB")
    print(f"Iterative FFT (N={len(signal_padded_iterative)}): Zeit = {fft_iterative_time:.6f} s, Speicher = {fft_iterative_memory / 1024:.2f} KiB")
